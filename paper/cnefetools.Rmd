...
abstract: >
  ...
  aggregations without transferring intermediate results to R, with an advantage that grows with dataset size and exceeds fifteenfold on São Paulo, the largest municipality in the country, where it also uses an order of magnitude less memory than the pure-R fallback.
draft: false
author:
...
where its peak memory is also an order of magnitude lower (see Section 5
for benchmarks).
...
before any network or disk operation begins.

This municipality-centric design is a real constraint for research
questions that span several municipalities, such as those posed at the
...
`cnefe_export()` writes a municipality to a location of the user's
choosing as Parquet (the default), CSV or gzipped CSV. The companion to
it is the `file` argument of `read_cnefe()`, which reads any of those
formats, as well as the ZIP exactly as IBGE publishes it, and therefore
skips retrieval entirely. Together they let a user keep an optimised
copy on whichever disk suits them, and they also make ingestion
independent of the download path, which matters if access to the
upstream server ever changes.
...
-   **`"duckdb"` (default)**: uses the \CRANpkg{duckdb} [@duckdb_r2024]
    ...
    \CRANpkg{duckspatial} R package handle the point-in-polygon join.
...
(one per UF, each containing a `geom_wkb` column with the geometries of census tract
...
\CRANpkg{smile} addresses the same change-of-support problem through a
...
\@ref(tab:tab-tracts-vars), alongside their variable code in the IBGE census tract aggregates
...
\CRANpkg{cnefetools} also exports four utility functions for documentation
...
what DuckDB allocates in C++. The speedups reported below differ from
those in the submitted version of this article because of this change in
measurement protocol rather than any change in the package, since the
earlier figures came from a single unreplicated run in which all DuckDB
cases preceded all pure-R cases.
...
5.  **DuckDB dual-backend**: a performance architecture that streams the
    ...
    municipality, where its peak memory is also an order of magnitude
    below the pure-R backend's.
...
The persistent caching layer in \CRANpkg{cnefetools} further mitigates this
