...
caches each downloaded ZIP in the user's application data directory
(via `tools::R_user_dir("cnefetools", "cache")`), creating the directory
on first use and persisting it across R sessions.
...
-   **`"duckdb"` (default)**: uses the \CRANpkg{duckdb} [@duckdb_r2024]
    ...
    The `zipfs` extension is always loaded to read CSV data directly from the cached
...
what DuckDB allocates in C++. The speedups reported below are smaller
than those in the submitted version of this article because of this
change in measurement protocol rather than any change in the package,
since the earlier figures came from a single unreplicated run in which
all DuckDB cases preceded all pure-R cases.

Figure \@ref(fig:fig-bench-cities) shows that DuckDB reduces the median
elapsed time for `cnefe_counts()` from 6.31 s to 5.55 s for Vitória da
Conquista-BA (approximately 200,000 addresses, 1.14× speedup), from
11.30 s to 5.56 s for Curitiba-PR (approximately 900,000 addresses,
2.03× speedup), and from 110.71 s to 8.33 s for São Paulo-SP
(approximately 5.7 million addresses, 13.3× speedup). The relative
...
three H3 resolutions. The DuckDB speedup is 1.88× at resolution 7, 2.02×
at resolution 9, and 1.02× at resolution 11. At finer resolutions (higher
...
flat across a 28-fold increase in input, rising only from 0.45 GB on
Vitória da Conquista-BA to 0.68 GB on São Paulo-SP, because it streams
...
between 0.90 and 1.20 across all three municipalities and both backends,
...
the most memory-hungry path in the package, reaching 4.31 GB on São
Paulo-SP, because an overlay against arbitrary user polygons cannot use
the H3 shortcut.

| Municipality | `tracts_to_h3()` | `tracts_to_polygon()` |
|:-----------------------------|:----------------:|:---------------------:|
| Vitória da Conquista-BA (\~200k) | 11.45 s, 0.67 GB | 7.05 s, 0.80 GB |
| Curitiba-PR (\~900k) | 8.56 s, 0.61 GB | 5.17 s, 1.11 GB |
| São Paulo-SP (\~5.7M) | 20.69 s, 1.90 GB | 20.35 s, 4.31 GB |
...
aggregates in about nine seconds within that budget, 12% slower than on
the full machine while using a third less memory, and the dasymetric
interpolation costs 35% more time, so neither degrades sharply as the
envelope tightens.

| São Paulo-SP (\~5.7M addresses) | full machine | 4 threads, 4 GB |
|:--------------------------------|:------------:|:---------------:|
| `cnefe_counts()` | 8.33 s, 0.68 GB | 9.37 s, 0.44 GB |
| `tracts_to_h3()` | 20.69 s, 1.90 GB | 27.85 s, 1.76 GB |
