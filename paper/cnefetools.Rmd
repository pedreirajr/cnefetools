...
abstract: >
  ...
  geocoded register from the 2022 census, published by the Instituto
  Brasileiro de Geografia e Estatística (IBGE), with 111.1 million
  records, one for each type of use found at an address. Although the CNEFE enables fine-grained spatial
  ...
  units using CNEFE dwelling points as ancillary data. The package provides a flexible dual-backend architecture for all computationally intensive operations.  The default DuckDB backend reads the cached data in-process and handles all spatial joins, H3 indexing, and
  aggregations without transferring intermediate results to R, with an advantage that grows with dataset size and exceeds fifteenfold on São Paulo, the largest municipality in the country, where its peak memory is also more than twelve times smaller than the pure-R fallback's.
draft: false
author:
...
[@ibge2026]. Since the 2010 Brazilian Census, the *Instituto
...
3.  **Land use mix indices**: compute a suite of established and
...
Because each CNEFE record corresponds to one address unit, the point count within a zone
...
\CRANpkg{h3jsr} [@h3jsr2023] and \CRANpkg{sf} [@pebesma2018], its
...
religious temples [@ibge_cnefe2024]. The first CNEFE was published
alongside the 2010 census, though the precision achieved at that time
was limited to the block-face level [@ibge_cnefe2024].
...
[@ibge_cnefe2024]. In the rest of this paper, *address* refers to a
...
dual DuckDB/R backend. The analytical functions — `cnefe_counts()`,
`compute_lumi()`, `tracts_to_h3()`, and `tracts_to_polygon()` — each
...
`read_cnefe()` covers only the first phase, returning the raw CNEFE table
...
[@lang2017] before any I/O (normalizing municipality codes, verifying
...
exists, and it reports the two failures differently. An unreachable
server points the user to their connection, while a missing file means
the layout has changed upstream, and the message asks the user to
report it. Either way, a ZIP obtained by other means can still be read
with `read_cnefe(file = ...)`, described below.
...
the `cache_dir` argument of the calling function, then the
`CNEFETOOLS_CACHE_DIR` environment variable, and finally
`tools::R_user_dir("cnefetools", "cache")`, which remains the default so
...
every function that reads data, and also by `clear_cache_muni()` and
`clear_cache_tracts()`, because a redirected cache the cleaners could
...
Caching can be disabled with `cache = FALSE`, in which case the data is
...
`cnefe_export()` writes a permanent copy to a location of the user's
choosing, and the `file` argument of `read_cnefe()` reads such a copy
...
-   **`"duckdb"` (default)**: uses the \CRANpkg{duckdb} [@duckdb_r2024]
    ...
    DuckDB decompresses gzip natively. For H3 aggregation, the `h3`
    extension assigns cell indices entirely in SQL, while for
    user-provided polygons, the `spatial` extension and the
    \CRANpkg{duckspatial} R package [@duckspatial2026] handle the
    point-in-polygon join.
    Only the final aggregated result is transferred to R. The `zipfs`
...
The census tract geometry assets required by `tracts_to_h3()` and
`tracts_to_polygon()` are stored as Parquet files (one per UF, each
containing a `geom_wkb` column with the geometries of census tract
...
caching via the \CRANpkg{piggyback} package [@piggyback2023].
...
the `data-raw/` directory of the package repository, so either can be
rebuilt or audited.

The index is built by `data-raw/cnefe_index_2022.R`, which takes the
...
The census tract assets are built by `data-raw/sc_assets_build.R`. Tract
geometries come from \CRANpkg{geobr}, and tract attributes come from
\CRANpkg{censobr} [@censobr2023], which repackages the IBGE census tract
aggregates, so the variable codes and table names listed in Table
...
from the release tag `sc-assets-v3`, and every value in them matches the
files originally published with the package, 3,144,868 values across
468,097 tracts in all 27 states. The SHA-256 checksum and size of each
file are recorded in `data-raw/sc_assets_checksums.csv`, so a user can
...
\CRANpkg{arrow} [@arrow2026] `Table` or an \CRANpkg{sf} point object. The code below
...
of the dataset previously read as an `arrow::Table`.

```{r cnefe-preview, echo=-3, eval=TRUE, cache=TRUE}
library(dplyr)

cnefe <- cnefetools::read_cnefe(
  code_muni = 2919207, verbose = FALSE
)
cnefe |>
...
previously read in `cnefe_sf`.

```{r fig-read-cnefe, echo=FALSE, eval=TRUE, cache=TRUE, fig.cap="Educational establishment addresses geocoded in Lauro de Freitas, Bahia.", fig.alt="Map of Lauro de Freitas municipality showing educational establishment address points as blue dots distributed across the urban area, with the municipal boundary outlined in black on a light grey background.", fig.width=6, fig.height=5, out.width="100%"}
library(geobr)
library(ggplot2)
library(sf)
library(dplyr)
...
# CNEFE educational establishments only (COD_ESPECIE == 4)
educational <- cnefetools::read_cnefe(
...
ggplot() +
  ...
  geom_sf(data = educational, size = 0.8, alpha = 0.7, color = "#2166AC") +
  theme_void() +
...
`addr_type1` to `addr_type8`. To illustrate both output modes, the examples
below count addresses per H3 hexagon at resolution 9 and per official
neighborhood boundary obtained from \CRANpkg{geobr} [@geobr2019].
...
When no `polygon` is supplied (the default), the function builds a
...
such as the A5 grid available in \CRANpkg{a5R} [@a5r2026] or IGEO7,
which is built on the ISEA projection [@kmoch2025]. \CRANpkg{cnefetools}
uses H3 because the DuckDB `h3` extension assigns cells inside the
...
When a `polygon` is supplied, the function performs a point-in-polygon
...
`polygon` is `NULL`, or the user-supplied polygons (in their original
CRS, or `crs_output` if specified) when one is given. Figure
...
found in @song2013, @massey2001, @pedreirajr2025 and
@pedreirajr2026bgbi.
...
@pedreirajr2026bgbi, but it differs from the multi-category definitions
of land use mix that are common in urban research [@song2013]. Users who
need the diversity of non-residential uses can compute it from the
counts by address type that `cnefe_counts()` returns.
...
`cnefe_counts()` does not apply this exclusion and reports these
addresses as `addr_type7`, so users can check how many there are in any
area of interest.
...
`polygon` is supplied, because it is meant to describe the city that
...
[@pedreirajr2026bgbi] for Lauro de Freitas at H3 resolution 8 and at the
neighborhood level.
...
\CRANpkg{cnefetools} addresses this need with `tracts_to_h3()` and `tracts_to_polygon()`, 
which perform dasymetric interpolation [@mennis2003] from census tract aggregates 
...
areal weighting. \CRANpkg{sf} [@pebesma2018], through
`st_interpolate_aw()`, and \CRANpkg{areal} [@prener2019], through
`aw_interpolate()`, both distribute source-zone totals in proportion to
...
\CRANpkg{smile} [@smile2025] addresses the same change-of-support problem through a
model-based approach. Source-zone values are treated as spatial averages
of a latent Gaussian random field, `fit_spm()` estimates the field
parameters from the source data, and `predict_spm()` aggregates
...
\CRANpkg{cnefetools}. The `pp_estimate()` function supports both areal
...
`pp_compare()` additionally facilitates accuracy assessment against
reference counts.
...
\@ref(tab:tab-tracts-vars), alongside their variable code and source
table in the census tract aggregates, as named by \CRANpkg{censobr}.

```{r tab-tracts-vars, eval=TRUE, echo=FALSE}
library(cnefetools)
knitr::kable(
  tracts_variables_ref[, c("var_cnefetools", "code_var_ibge", "table_ibge")],
  col.names = c("Variable", "Variable code", "Source table"),
  caption = if (knitr::is_latex_output()) {
    paste0(
      "Variables supported by \\texttt{tracts\\_to\\_h3()} and",
      " \\texttt{tracts\\_to\\_polygon()}, with their variable codes and",
     " source tables in the census tract aggregates, as named by censobr. Portuguese descriptions",
      " are available in \\texttt{tracts\\_variables\\_ref\\$desc\\_var\\_ibge}."
    )
  } else {
    paste0(
      "Variables supported by `tracts_to_h3()` and",
      " `tracts_to_polygon()`, with their variable codes and",
      " source tables in the census tract aggregates, as named by censobr. Portuguese descriptions",
...
1.  **DuckDB setup**: an in-memory DuckDB database is opened and two
    extensions are loaded: `h3` (for H3 cell assignment in SQL) and
    `spatial` (for `ST_Within` point-in-polygon joins). Reading the
...
3.  **CNEFE points**: DuckDB streams the semicolon-delimited CSV
...
5.  **Aggregation**: the `cnefe_alloc` view is grouped by target spatial
    ...
    with `SUM`, while `avg_inc_resp` is aggregated with `AVG` (mean of
...
access, cache management and data export. `cnefe_dictionary()` opens the official
...
`cnefe_export()` writes a municipality to a directory of the user's
...
cache, which the functions above are meant to empty. The `file`
argument of `read_cnefe()` reads any of those formats, as well as the
...
2.  **Streaming aggregation**: DuckDB decompresses the cached gzipped
...
the whole R process, obtained through the `ps` package rather than
...
elapsed time for `cnefe_counts()` from 4.69 s to 3.79 s for Vitória da
...
```{r fig-bench-memory, echo=FALSE, eval=TRUE, out.width="70%", fig.align="center", fig.alt="Grouped bar chart comparing peak memory in gigabytes for cnefe_counts() at H3 resolution 8 across three municipalities of increasing size. For each municipality, two bars represent the DuckDB and pure-R backends; the DuckDB bars stay nearly constant at or below 0.7 GB while the pure-R bars grow with municipality size, reaching about 9 GB for São Paulo.", fig.cap=paste0("Peak memory (GB) for ", cfmt("cnefe_counts()"), " at H3 resolution~8, by municipality size, comparing the DuckDB and pure-R backends.")}
knitr::include_graphics("figures/bench-memory.png")
```
...
constraint is memory. DuckDB sets `memory_limit` to 80% of installed RAM
...
## Other heavy functions

`compute_lumi()` performs the same scan, the same H3 assignment and the
same grouped aggregation as `cnefe_counts()`, and then adds arithmetic
...
than twice the size of Paraná's. In addition, `tracts_to_polygon()` is
...
| Municipality | `tracts_to_h3()` | `tracts_to_polygon()` |
|:-----------------------------|:----------------:|:---------------------:|
...
: (#tab:tab-bench-tracts) Median elapsed time and peak memory for the
...
the `cnefetools.duckdb_config` option makes settable. São Paulo-SP
...
: (#tab:tab-bench-modest) Median elapsed time and peak memory on São
Paulo-SP under a constrained DuckDB configuration, against the same
calls with the machine's full resources.
...
and memory. The pure-R backend (`backend = "r"`) remains available for
...
5.  **DuckDB dual-backend**: a performance architecture that streams the
...
the package offers against that risk is that `read_cnefe(file = ...)`
...
codes and combining the results, and @pedreirajr2026 gives a worked
...
H3 grid built by `cnefe_counts()` and `compute_lumi()` is derived from
...
that `compute_lumi()` uses as its reference is likewise computed per
...
such questions, the counts returned by `cnefe_counts()` keep all eight
address types and can feed a multi-category index directly.

A related design choice concerns what `read_cnefe()` returns. It
returns an Arrow table or an `sf` object held in memory, rather than a
...
`cnefe_export()` and query the file lazily with `arrow::open_dataset()`
or DuckDB.
