...
abstract: >
  ...
  to millions of records. \pkg{cnefetools} addresses both through
...
spatial units, using CNEFE dwelling points as ancillary data. Two existing R tools
address related problems: `sf::st_interpolate_aw()` [@pebesma2018] and
\CRANpkg{areal} [@prener2019] implement areal-weighted interpolation, which
distributes source-zone totals in proportion to the overlap area between source and
target polygons, implicitly assuming a uniform spatial distribution of the attribute
within each source zone. By contrast, `tracts_to_h3()` and `tracts_to_polygon()`
use geocoded dwelling points as ancillary data, concentrating allocated mass where
residential addresses actually are rather than spreading it uniformly across the
tract. This produces more accurate estimates in municipalities with heterogeneous
land use, where population density varies sharply within census tracts. The set of supported
