---
title: "Dasymetric interpolation with tracts_to* functions"
editor_options:
  markdown:
    wrap: 72
---
...
spatial granularities: hexagonal grids for standardized spatial
...
tract-level totals to individual dwelling points identified in the CNEFE
and then aggregate these to the target spatial units. This approach
preserves the heterogeneous distribution of dwellings within each tract,
yielding more accurate estimates than simple areal interpolation.
...
ggplot(ftl_h3_plot, aes(x = pct_preta, y = avg_inc_resp)) +
  geom_point(alpha = 0.3, size = 3) +
  geom_smooth(method = "loess", se = TRUE, color = "steelblue") +
  scale_x_continuous(labels = scales::percent_format()) +
  labs(
    x = "Black population share (race_preta / pop_ph)",
    y = "Average income of household heads (R$)"
  ) +
  theme_minimal()
```
...
sp_zones_census <- tracts_to_polygon(
  code_muni = 3550308,
  polygon = sp_zones_muni |> sf::st_make_valid(), # Correcting invalid geometries
