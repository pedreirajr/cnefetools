...
computes a suite of LUM indices from CNEFE data considering a binary comparison
of residential vs. non-residential uses, aggregated either to H3 hexagonal cells
or user-provided polygons.
...
$P = 0.9$, which is typical of Brazilian municipalities.

```{r, fig.width = 8, fig.height = 4}
P <- 0.8 # citywide residential proportion
...
## Generating p and q values between 0 and 1
p <- seq(0, 1, by = 0.01)
q <- 1 - p

## Index dataframe
df_ind <- data.frame(p = p, q = q) |>
  mutate(
    EI = {
      m <- cbind(p, q)
      m[m == 0] <- NA # Assign NA values when p or q = 0 to avoid log(0)
...
that point, whereas BAL peaks at $p_{i} = P = 0.8$ (the citywide reference),
...
Index of Concentration at the Extremes (ICE), the Herfindahl--Hirschman
Index (HHI), and an adapted directional HHI (aHHI). All indices
are returned in a single output, allowing comprehensive comparisons
