...
-   [**spatial**](https://duckdb.org/docs/extensions/spatial/overview.html): performs spatial joins (e.g., point-in-polygon) in SQL, used when aggregating to user-provided polygons or performing dasymetric interpolation.
-   [**zipfs**](https://duckdb.org/docs/extensions/zipfs.html): reads CSV files directly from cached ZIP archives, avoiding the need to extract files to disk.
-   [**h3**](https://duckdb.org/docs/extensions/h3.html): assigns geographic coordinates to [H3 hexagonal grid](https://h3geo.org/) cells entirely inside DuckDB.
