...
If you prefer to work with spatial objects, `read_cnefe()` can return an
`sf` object by setting `output = "sf"`. The example below reads CNEFE
data for Salvador, filters only religious facilities
...
`tracts_to_h3()` implements a dasymetric interpolation workflow that
transfers census tract totals to address-level points (CNEFE), and then
aggregates the allocated values to an H3 grid.
...
`tracts_to_h3()` prints an execution timing summary (when
`verbose = TRUE`) and may emit a warning with a diagnostic report. The
diagnostics report includes, for each requested variable:
