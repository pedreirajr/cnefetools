...
`sf` object by setting `output = "sf"`.  
The example below reads CNEFE data for Salvador, filters only religious
facilities (`COD_ESPECIE == 8`), and plots them with `ggplot2`:
...
> address points.  
> Plotting all coordinates at once can be slow and memory intensive, so
