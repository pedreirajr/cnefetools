# Authors and Citation

## Authors

- **Jorge Ubirajara Pedreira Junior**. Author, maintainer, copyright
  holder. [](https://orcid.org/0000-0002-8243-5395)

- **Bruno Mioto**. Author.

- **Kaio Cunha Pedreira**. Contributor.

## Citation

Source:
[`DESCRIPTION`](https://github.com/pedreirajr/cnefetools/blob/sc-assets-v3/DESCRIPTION)

Pedreira Junior J, Mioto B (2026). *cnefetools: Access and Analysis of
Brazilian CNEFE Address Data*. R package version 0.3.0.9000,
<https://github.com/pedreirajr/cnefetools>.

    @Manual{,
      title = {cnefetools: Access and Analysis of Brazilian CNEFE Address Data},
      author = {Jorge Ubirajara {Pedreira Junior} and Bruno Mioto},
      year = {2026},
      note = {R package version 0.3.0.9000},
      url = {https://github.com/pedreirajr/cnefetools},
    }
